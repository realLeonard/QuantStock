...

{
  ...
  "fund_picture": {
    ...
    "margin_summary": "两融杠杆资金结论（必须含: 融资余额 X 万亿/亿、1Y 分位 X%、日变化 ±X 亿、连续 ±N 日；再给出定性判断: 顶部警示/加仓信号/企稳/撤离等）"
  },
  "important_news": [
    {
      "segment": "pre_market/intraday/post_market",
...
10. fund_picture.margin_summary：基于 margin_data 撰写，解读两融杠杆资金。定性判断口径：
    - 余额 1Y 分位 ≥ 70% + 日变化转负或连续净减 → "顶部警示，杠杆撤离"
    - 余额 ≥ 70% 分位 + 日变化仍正 + 连续净增 → "杠杆加仓，主升段特征"
    - 余额 ≤ 30% 分位 + 日变化转正或连续净增 → "底部企稳信号，聪明钱试探"
    - 连续净增 ≥ 3 日 → 强调"趋势资金在场"
    - 单日净减 > 100 亿 → "风险事件/冰点信号"
