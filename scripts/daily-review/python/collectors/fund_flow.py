    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                print(f'  [warn] 获取板块资金流向失败(第{attempt}次), {e}, {5 * attempt}s 后重试...')
                time.sleep(5 * attempt)
            else:
                print(f'  [warn] 获取板块资金流向失败(已重试{_MAX_RETRIES}次): {e}')
...
def collect_stock_fund_flow(date_str: str) -> dict:
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if attempt < _MAX_RETRIES:
                print(f'  [warn] 获取个股资金流向失败(第{attempt}次), {e}, {5 * attempt}s 后重试...')
                time.sleep(5 * attempt)
            else:
                print(f'  [warn] 获取个股资金流向失败(已重试{_MAX_RETRIES}次): {e}')
